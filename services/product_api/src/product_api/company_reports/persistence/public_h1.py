...
async def list_report_resolution_records(session: AsyncSession, inn: str) -> list[ReportResolutionRecord]:
    rows = (await session.execute(select(CompanyReportRecord, CompanyReportSubject).join(CompanyReportSubject, CompanyReportSubject.id == CompanyReportRecord.subject_id).where(CompanyReportSubject.normalized_identifier == inn).order_by(desc(CompanyReportRecord.created_at), desc(CompanyReportRecord.id)))).all()
