...
async def get_publication_resolution_record(session: AsyncSession, inn: str) -> PublicationResolutionRecord | None:
    row = (await session.execute(
        select(CompanyReportPublication, CompanyReportSubject, CompanyReportRecord)
        .join(CompanyReportSubject, CompanyReportSubject.id == CompanyReportPublication.subject_id)
        .outerjoin(CompanyReportRecord, and_(
            CompanyReportRecord.id == CompanyReportPublication.report_id,
...
async def list_report_resolution_records(session: AsyncSession, inn: str) -> list[ReportResolutionRecord]:
    rows = (await session.execute(select(CompanyReportRecord, CompanyReportSubject).join(CompanyReportSubject, CompanyReportSubject.id == CompanyReportRecord.subject_id).where(CompanyReportSubject.normalized_identifier == inn, CompanyReportRecord.writer_profile == "h1_legacy_writer_v2", CompanyReportRecord.presentation_contract == "company_public_h1_v1", CompanyReportRecord.rollout_generation == 0, CompanyReportRecord.report_version.in_(("1", "2"))).order_by(CompanyReportRecord.generated_at.desc().nullslast(), desc(CompanyReportRecord.id)))).all()
