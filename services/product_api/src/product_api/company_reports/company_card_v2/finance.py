...
def build_chart_facts(basis: FinanceBasisV1) -> ChartFactsV1:
    ...
    payload = {"version": "company_card_chart_facts_v1", "facts": [item.model_dump(mode="json") for item in facts]}
...
def _shares(equity: Decimal, debt: Decimal, denominator: Decimal) -> tuple[Decimal, Decimal]:
    equity_share = _ratio(equity, denominator)
    debt_share = Decimal("100") - equity_share
    return equity_share, debt_share
