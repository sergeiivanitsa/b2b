...
export function CompanyReportInnForm({ id, value, compact = false, disabled = false, onChange, onSubmit }: Props) {
  ...
  return (
    <form className={`company-entry-form${compact ? ' company-entry-form--compact' : ''}`} onSubmit={submit} noValidate>
      ...
      <div className="company-entry-form__controls">
        <input
          ...
          disabled={disabled}
        />
        <button type="submit" disabled={disabled}>{disabled ? 'Открываем…' : 'Проверить'}</button>
      </div>
