...
function ChartMark({ id, label, tooltipId, x, y, width = 12, height = 12, open }: { id: string; label: string; tooltipId: string; x: number; y: number; width?: number; height?: number; open: (item: Disclosure) => void }) { const visibleWidth=Math.max(4,width), visibleHeight=Math.max(4,height), hit=44; return <g><rect aria-hidden="true" x={x} y={y} width={visibleWidth} height={visibleHeight} fill="#EE5A2A" /><rect data-h2-chart-mark={id} tabIndex={0} role="button" aria-describedby={tooltipId} aria-label={label} x={x+visibleWidth/2-hit/2} y={y+visibleHeight/2-hit/2} width={hit} height={hit} fill="transparent" onFocus={() => open({ id, text: label })} onMouseEnter={() => open({ id, text: label })} onTouchStart={() => open({ id, text: label })} onPointerDown={() => open({ id, text: label })} /></g> }
...
function F3({ view, open, tooltipId }: { view: PublicH2FinanceF3Dto; open: (item: Disclosure) => void; tooltipId: string }) { return <div className="company-public-h2__chart-panels"><F3Panel label="Выручка" view={view} metric="revenue" open={open} tooltipId={tooltipId} /><F3Panel label="Активы" view={view} metric="assets" open={open} tooltipId={tooltipId} /></div> }
function F4({ view, open, tooltipId }: { view: PublicH2FinanceF4Dto; open: (item: Disclosure) => void; tooltipId: string }) { const axis=view.axis; if (axis === null || view.mode !== 'per_100') return null; const values = [['Валовая прибыль', view.gross_per_100_decimal], ['Прибыль от продаж', view.operating_per_100_decimal], ['Чистая прибыль', view.net_per_100_decimal]] as const; return <AxisSvg label="Прибыль на 100 рублей выручки">{values.map(([metric, value], index) => value === null ? null : <ChartMark key={metric} id={`f4-${index}`} tooltipId={tooltipId} label={markLabel(metric, year(view.year), per100(value), 'available')} x={80 + index * 135} y={BASE - decimalCoordinate(value, axis.axis_min_decimal, axis.axis_max_decimal, 120)} open={open} />)}</AxisSvg> }

function Layer({ hostId, children }: { hostId: HostId; children: (open: (item: Disclosure) => void, tooltipId: string) => ReactNode }) { const [disclosure, setDisclosure] = useState<Disclosure>(null); const root=useRef<HTMLElement>(null); const instance=useId().replaceAll(':',''); const tooltipId = `${hostId}-tooltip-${instance}`; useEffect(()=>{const close=(event:PointerEvent)=>{if(root.current!==null&&!root.current.contains(event.target as Node))setDisclosure(null)};document.addEventListener('pointerdown',close);return()=>document.removeEventListener('pointerdown',close)},[]); return <section ref={root} className="company-public-h2__chart-layer" onMouseLeave={() => setDisclosure(null)} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDisclosure(null) }} onKeyDown={event => { if (event.key === 'Escape') setDisclosure(null) }}>{children(setDisclosure, tooltipId)}{disclosure && <p className="company-public-h2__chart-tooltip" id={tooltipId} role="tooltip">{disclosure.text}</p>}</section> }
...
export function FinanceChartForHost({ dto, hostId, onError }: { dto: CompanyPublicH2; hostId: HostId; onError: () => void }) {
  const child = hostId === 'finance-f1' && dto.blocks.finance_f1 ? (open: (item: Disclosure) => void, tooltipId: string) => <F1 view={dto.blocks.finance_f1!} open={open} tooltipId={tooltipId} />
    : hostId === 'finance-f2' && dto.blocks.finance_f2 ? (open: (item: Disclosure) => void, tooltipId: string) => <F2 view={dto.blocks.finance_f2!} open={open} tooltipId={tooltipId} />
      : hostId === 'finance-f3' && dto.blocks.finance_f3 ? (open: (item: Disclosure) => void, tooltipId: string) => <F3 view={dto.blocks.finance_f3!} open={open} tooltipId={tooltipId} />
        : hostId === 'finance-f4' && dto.blocks.finance_f4 ? (open: (item: Disclosure) => void, tooltipId: string) => <F4 view={dto.blocks.finance_f4!} open={open} tooltipId={tooltipId} /> : null
