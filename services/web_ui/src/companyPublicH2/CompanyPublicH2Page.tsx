...
function LegacyArbitrationSurface({ blocks }: { blocks: PublicH2BlocksDto }) {
  const items: readonly (readonly [string, PublicH2ViewDto | null])[] = [
    ['arbitration_a1', blocks.arbitration_a1], ['arbitration_a2', blocks.arbitration_a2],
    ['arbitration_a3', blocks.arbitration_a3], ['arbitration_a4', blocks.arbitration_a4],
    ['arbitration_a5', blocks.arbitration_a5],
  ]
...
export function CompanyPublicH2Page({ dto }: { dto: CompanyPublicH2 }) {
  ...
  return <>
    ...
    <section id="arbitration"><h2>Арбитраж</h2><h3>Покрытие</h3><ul>{['arbitration_a1', 'arbitration_a2', 'arbitration_a3', 'arbitration_a4', 'arbitration_a5'].map(id => <CoverageRow item={coverageById(coverage, id)} key={id} />)}</ul>{arbitrationPolicyV3 ? <ArbitrationFacts dto={dto} /> : <LegacyArbitrationSurface blocks={blocks} />}</section>
