...
export function validateCompanyPublicH2Semantics(root:Obj):void {
 ...
 coverage.forEach((q,n)=>{const x=rec(q,`coverage[${n}]`),codes=ar(x.limitation_codes,'limitation_codes').map((z,j)=>s(z,`code[${j}]`)),state=s(x.state,'state');if(!unique(codes)||(!['available','missing'].includes(state)&&codes.length===0))fail('invalid coverage')})
