...
`codex/iteration-25-company-card-v2-qa-rollout-refresh`, created clean from the
exact post-prerequisite `origin/main`; the initial draft branch was not rebased
or reset.

Статус: `IMPLEMENTATION APPROVED — IN PROGRESS`

Initial planning authorization: owner command to proceed on `2026-08-27`

Refresh authorization: owner command to proceed on `2026-08-28`
...
6. Merged implementation contracts iterations 20–24, including the
   iteration-20 presentation lifecycle continuation specification, plan and
   baseline evidence.
7. `iteration-24-post-merge-postgresql-acceptance-v1.md`.
8. Iteration-24 owner decision D1–D6 для current visible arbitration policy.
...
Выполнено на refreshed base:

1. Iteration-20 continuation merged через PR `#150`, reconciliation — PR
   `#151`. Exact lifecycle/OpenAPI subset проходит `50` tests; disposable
   PostgreSQL Targeted/Full проходят `117/290` без failures/errors/skips.
2. Iteration-24 acceptance merged и reconciled через PR `#152`. Exact runner
   повторно завершился `2 + 79 passed` без failures/errors/skips; owned Docker
   labels до и после запусков пусты.
3. Создан новый clean worktree/feature branch от exact
   `31b299ac88b5fac7d5c04082324fb122d63db7e7`; initial draft не rebased/reset.
   Зафиксированы bounded delta `886f207...31b299ac`, новый inventory, immutable
   hashes и полный Stage 0 baseline.

Наблюдавшиеся `2 + 79 passed` остаются достоверным point-in-time результатом,
но текущий iteration-24 runner проверяет только exit code и потому не будет
повторно использоваться как iteration-25 no-skip gate. Iteration 25 не меняет
тот исторический runner script. Его migration test получает только
forward-head compatibility edit: два generic `upgrade(..., "head")` заменяются
на explicit `upgrade(..., "0018_company_card_v2_arbitration")`, без удаления
или ослабления iteration-24 assertions. Собственный iteration-25 acceptance
runner/checker запускает этот exact-0018 phase с отдельным JUnit, затем явно
поднимает roundtrip DB `0018 -> head`, проверяет, что head — revision `0019`, и
только после этого запускает affected integration phase с отдельным JUnit. Он
fail closed на missing/stale/malformed XML, zero tests и любом
failure/error/skip до того, как результат используется как gate.

Forward-head amendment review refreshed specification/plan завершён с
`VERDICT: APPROVED`.

Owner implementation approval полной reviewed specification/plan получен
2026-08-28. Если regression обнаружит defect,
принадлежащий iteration 20–24, он оформляется как отдельный prerequisite и не
скрывается внутри iteration-25 cross-layer scope.
...
crawler and browser read performs only read-only database work and local
deterministic rendering with bounded retained application memory. A sitemap
may scan its finite candidate stream in bounded windows to apply the full
canonical validator, but never materializes the complete publication set. It
performs no:
...
The staged-pointer write and read boundaries accept only an exact H2
staged/noindex shape: legacy `projection_scope=null` or
`staged_publication`, `indexable=false`, and null canonical/lastmod. They reject
every `active_publication` pin, including active noindex, and every indexable
pin. A forged or later-corrupted staged pointer fails closed and cannot expose
an active/indexable document through the staged/latest preview resolver.
...
decision_digest)` binding prevents local decision mismatch. The assignment
table additionally exposes unique `(id, subject_id)`. The journal's existing
scalar assignment foreign key is replaced, without changing either column, by
`(assignment_id, subject_id)` → assignment `(id, subject_id)` with the same
`ON DELETE CASCADE` behavior. Together with the existing exact subject/pin
foreign key this makes cross-subject assignment and pin mixtures impossible at
the database boundary. Upgrade validates every legacy journal/assignment pair
and fails without rewriting data if a mismatch exists.
...
- `0019` adds no sitemap eligibility bit, materialized registry or backfill;
  sitemap eligibility remains a read-time full-validation result.
- Downgrade is tested only on disposable legacy/no-new-transition states. In
  one PostgreSQL transactional-DDL transaction it first acquires
  `SHARE ROW EXCLUSIVE` table locks in this exact order:
  `company_card_v2_rollout_decisions`,
  `company_report_presentation_assignments`,
  `company_report_presentation_pins`,
  `company_report_presentation_assignment_journal`. It holds them through
  guard and DDL, then refuses to discard
  **any** pin with non-null `projection_scope`, any journal row with a new
  audit field or any global decision binding. A race test proves that a
  concurrent writer is either included in the guarded state and causes
  refusal or runs only after the completed downgrade and cannot silently lose
  new-schema data.
...
Sitemap persistence is bounded in application memory. Each request uses one
read-only `REPEATABLE READ` transaction. One SQL overlay applies precedence
and returns at most one candidate tuple per subject: no assignment selects the
current H1 publication tuple; any assignment selects only its exact
assignment/pin/report dependency tuple. Assignment presence suppresses H1
**before** validation, so missing, noindex or corrupt assigned H2 can never
fall back to H1.

Candidates are streamed in keyset windows of at most `100`, ordered by
`(normalized_inn, coalesce(selected_canonical_path, ''), subject_id)`. Each
candidate, including assigned noindex, is evaluated by the same complete pure
H1/H2 validator used by canonical resolution; sitemap does not call the
resolver again or select another assignment/latest report. The index scans the
finite stream and retains only an eligible integer count. Chunk N scans that
same logical stream, skips exactly `(N - 1) * chunk_size` **validated eligible**
entries and retains at most one output chunk. SQL `OFFSET` over raw candidates
and SQL-only eligibility/count are forbidden because they would create holes
around invalid rows. Memory is bounded by one 100-row validation window, one
validation state and one output chunk; index/deep/out-of-range work is openly
`O(candidate_count)`. Empty/out-of-range chunks are stable. A storage/system
failure aborts with a safe error and never emits partial XML.

Changing the internal window size or insertion order must not change XML
bytes. In a fixed transaction snapshot index/chunk use identical precedence,
ordering and validation. A request concurrent with assignment/rollback sees a
complete before-view; the next request sees the committed after-view. Legacy
no-assignment H1 output/order/lastmod remain byte-equivalent to current valid
H1 behavior.
...
The existing SPA >500 kB warning is a build baseline. npm advisory results are
dated external observations bound to an exact lock identity and registry, not
an immutable baseline; they are neither silently fixed nor treated as a
waiver. New dev dependencies must not enter either production bundle.
...
- forward-head compatibility: preserved iteration-24 migration assertions run
  against explicit `0018` using the frozen `i24_guard_<12hex>` /
  `i24_roundtrip_<12hex>` / `i24u<12hex>` identity shape inside the
  iteration-25-owned disposable container, after which the same roundtrip DB
  upgrades to exact `0019/head` before the affected integration phase;
...
- deterministic downgrade locks plus a concurrent-writer guard/DDL race;
- assignment `(id, subject_id)` identity and cross-subject journal rejection;
- H1/H2 pin coexistence;
- staged → active noindex/indexable pin append;
- staged-pointer write/read rejection of active/indexable H2 pins;
...
- sitemap/canonical/Claims H1 → H2 → H1 continuity, full-validator parity,
  late-corruption exclusion, packed valid/invalid chunk boundaries, bounded
  100-row windows and repeatable-read before/after views;
...
3. No previous iteration gap was silently absorbed; the historical
   iteration-24 runner script remains unchanged and is not reused as a no-skip
   gate. Its migration test only replaces two stale `head` aliases with exact
   `0018`; all assertions remain. The iteration-25 runner then proves
   `0018 -> 0019/head` and enforces JUnit proof for both prerequisite phases.
...
9. Sitemap/canonical/robots expose exactly the active assignment, use the same
   complete H1/H2 validation semantics and never duplicate H1/H2.
