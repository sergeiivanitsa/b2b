...
foreach ($needle in @('proxy_intercept_errors on;', 'error_page 404 = @company_spa;', 'location @company_spa', 'location ^~ /company-lab/', 'location @company_lab_spa', 'add_header X-Robots-Tag "noindex, nofollow" always;', 'location = /robots.txt', 'location = /sitemaps/index.xml')) {
...
if ($config -notmatch 'location \^~ /company-lab/ \{[\s\S]*try_files \$uri @company_lab_spa;[\s\S]*add_header X-Robots-Tag "noindex, nofollow" always;') { throw 'CompanyReport lab routes must keep noindex before the SPA fallback' }
if ($config -notmatch 'location @company_lab_spa \{[\s\S]*internal;[\s\S]*rewrite \^ /index\.html break;[\s\S]*add_header X-Robots-Tag "noindex, nofollow" always;') { throw 'CompanyReport lab SPA fallback must preserve noindex in its final response context' }
